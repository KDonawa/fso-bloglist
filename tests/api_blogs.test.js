...
describe("Token authentication", () => {
  ...
  test("Valid token in POST request should return with status 201", async () => {
    ...
    await api.post("/api/blogs").set("Authorization", `bearer ${authenticatedUser.token}`).send(newBlog).expect(201);
  });
...
describe("Deleting and updating db entries", () => {
  ...
  test.skip("updating an existing blog succeeds with status code 200 if id is valid", async () => {
    const blogsAtStart = await blogsInDb();
...
describe("Adding blogs to db", () => {
  ...
  test("fails with status code 400 if missing title or url", async () => {
    ...
    await api
      ...
      .expect(400);
    await api.post("/api/blogs").set("Authorization", `bearer ${authenticatedUser.token}`).send({}).expect(400);
  });
});
